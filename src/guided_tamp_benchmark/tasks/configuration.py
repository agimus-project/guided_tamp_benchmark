...
class Configuration:
    """The configuration class consists of robot joint angles and the list of poses of manipulated objects. It
    ...
    def __init__(self, q: Union[List[float], np.array], poses: Union[List[np.array], np.array]) -> None:
        ...
        given by a list of N 4x4 transformations or by np.array of size Nx4x4"""
        super().__init__()
        self.q = np.asarray(q)
    ...
    def to_numpy(self) -> np.array:
        """Return numpy array of the whole configuration in a form [robot_joints, obj1_pos, obj1_quaternion, ...]"""
    ...
    def from_numpy(config: np.array, robot_ndofs: int):
        """Fill in the internal configuration from the configuration in a form shown in @method to_numpy"""
        config = config.copy()
    ...
    def distance(self, other) -> Tuple[float, float, float]:
        """Return 3 dimensional tuple containing distance of robots, linear distance of objects, and rotational
        distance of objects. """
